...
int main(int argc, char **argv)
{
    ...
    {
        ROS_ERROR_STREAM("Input parameter sequence incorrect, [bag2video] exit.");
        return -1;
    }
    ...
    // else
    // {
    //     std::cerr << "Failed to match the image size. frame_width:" << frame_width << ", frame_height:" << frame_height << std::endl;
    //     return -1;
    // }
    ...
    if(getcwd(path_t, sizeof(path_t)) != NULL)
    {
        bagPath = path_t;
        bagPath += "/" + bagName;
        std::cout << "[bag2video INFO]:" << " --bagPath:" << bagPath << std::endl;
    }
    else
    {
    ...
    vWriter.release();
    bag.close();
    std::cout << "\nbag2video complished extract: [--compressed images " << frames_cpr << std::endl
              << "\033[?25h"; // Show cursor.
